...
@Component({selector:'app-audit-page',standalone:true,imports: [A11yModule, StaffPageHeaderComponent, CommonModule,FormsModule],templateUrl:'./audit-page.component.html',styleUrl:'./audit-page.component.scss',changeDetection:ChangeDetectionStrategy.OnPush})
export class AuditPageComponent {
  ...
  @HostListener('document:keydown',['$event']) keyboard(event:KeyboardEvent){if(!(this.detailLoading()||this.detail()||this.detailError()))return;if(event.key==='Escape'){event.preventDefault();this.close();return;}}
